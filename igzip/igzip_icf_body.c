                           ...
                           struct deflate_icf *match_lookup)
{
    ...
    while (next_in < end_processed)
    {
        ...
        if (len >= 8 + LEN_OFFSET)
        {
            match_len = compare ((next_in + 8) - dist, next_in + 8, end_in - (next_in + 8)) +
                        LEN_OFFSET + 8;

            while (match_len > match_lookup->lit_len && match_len >= LEN_OFFSET + SHORTEST_MATCH)
            {
                write_deflate_icf (match_lookup, match_len > LEN_MAX ? LEN_MAX : match_len,
                                               ...
                                               struct deflate_icf *matches_end)
{
    ...
    while (matches_next + 1 < matches_end && level_buf->icf_buf_next + 1 < icf_buf_end)
    {
        ...
        else if (lit_len2 >= LEN_START)
        {
            ...
            store_native_u32 ((uint8_t *) (level_buf->icf_buf_next + 1), (uint32_t) (code >> 32));
#endif
